...
## 現在の実装状況
- バックエンド: タスクの全件取得・ステータス絞り込み検索・新規作成API(`GET/POST /api/tasks`)
- フロントエンド: バックエンドAPIから取得したタスクをボード画面(未着手/進行中/完了の3列)に表示(表示のみ)
- カードの追加・削除・ドラッグ&ドロップによる更新は、`prototype/` のHTML/CSS/JSモックでは動作しますが、本実装(React側)への反映は今後の対応予定です
